...
$(KERNEL_CONFIG): $(KERNEL_OUT)
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) $(KERNEL_DEFCONFIG)
...
ifeq ($(KERNEL_ENABLE_EXFAT), m)
	...
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) vmlinux
...
ifeq ($(HTC_DEBUG_FLAG), DEBUG)
	...
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) SUBDIRS=$(BUILD_PATH)/kernel/fs/$(KERNEL_EXFAT_PATH)/objects modules
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) SUBDIRS=fs/$(KERNEL_EXFAT_PATH)/objects INSTALL_MOD_PATH=../../$(KERNEL_MODULES_INSTALL) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) modules_install
...
ifeq ($(TARGET_BUILD_VARIANT), user)
	...
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) SUBDIRS=$(BUILD_PATH)/kernel/fs/$(KERNEL_EXFAT_PATH)/objects-user modules
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) SUBDIRS=fs/$(KERNEL_EXFAT_PATH)/objects-user INSTALL_MOD_PATH=../../$(KERNEL_MODULES_INSTALL) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) modules_install
	cp kernel/fs/$(KERNEL_EXFAT_PATH)/objects-user/texfat.ko $(KERNEL_MODULES_OUT)/
	mkdir -p $(TARGET_OUT)/bin/
	cp -rf kernel/fs/$(KERNEL_EXFAT_PATH)/bin/* $(TARGET_OUT)/bin/
else
	# Build exfat modules for NonDebug-USERDEBUG
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) SUBDIRS=$(BUILD_PATH)/kernel/fs/$(KERNEL_EXFAT_PATH)/objects-userdebug modules
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) SUBDIRS=fs/$(KERNEL_EXFAT_PATH)/objects-userdebug INSTALL_MOD_PATH=../../$(KERNEL_MODULES_INSTALL) ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) modules_install
...
ifeq ($(MOCANA_FIPS_MODULE), true)
	...
	$(MAKE) -C kernel ARCH=arm CROSS_COMPILE=$(TARGET_KERNEL_TOOLS_PREFIX) PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) mrproper
endif
ifeq ($(KERNEL_ENABLE_EXFAT), m)
...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT) $(KERNEL_CONFIG)
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) headers_install

kerneltags: $(KERNEL_OUT) $(KERNEL_CONFIG)
	$(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) tags

kernelconfig: $(KERNEL_OUT) $(KERNEL_CONFIG)
	env KCONFIG_NOTIMESTAMP=true \
	     $(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) menuconfig
	env KCONFIG_NOTIMESTAMP=true \
	     $(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=arm PRIVATE_RCMS_NAME=$(PRIVATE_RCMS_NAME) savedefconfig
